...
> Программа предназначена для обучения и демонстрации причинно-следственных связей. Не используйте результаты в проектных расчётах промышленного абсорбера.
...
3. Включите возмущение состава, расхода или оба возмущения и введите относительное изменение. Например, `0.10` означает увеличение на 10%, а `-0.10` — уменьшение на 10%.
...
Главное окно состоит из верхней информационной панели, боковой навигации, панели параметров выбранного раздела, рабочей области с показателями и графиками, общих кнопок расчёта и нижней строки состояния. Минимальный размер окна — 1 100 × 680 пикселей; длинные панели прокручиваются колёсиком мыши.
...
| Строка контекста | Показывает активный сценарий и режим регулятора. После расчёта также показывает максимальное отклонение и время установления. |
...
| **«v1.4.2 · Python 3.14»** | Версия приложения и целевая версия Python. |

<a id="sidebar"></a>
### 2.2. Боковая навигация

Каждая кнопка открывает свою панель слева от графиков. Выбранный раздел подсвечивается.
...
Если поле заполнено неверно, приложение открывает нужный раздел, подсвечивает ошибочное поле и не выполняет расчёт.
...
| Элемент | Назначение |
|---|---|
| **«Параметры модели»** | Открывает отдельное окно с базовыми расходами и концентрациями. Его элементы описаны в [разделе 13](#model-dialog). |
...
Смена цепи сбрасывает текущий расчёт и устанавливает базовое значение выбранной цепи в поле задания регулятора.
...
| **«Постоянная времени T, с»** | Инерционность объекта. Значение должно быть больше нуля. Примерно за `T` секунд отклик достигает 63% полного изменения после начала реакции. |
...
- **импульсное** — в течение заданной длительности сигнал изменяется по полуволне синуса и возвращается к нулю;
...
| **«Максимальное отклонение»** | Наибольший модуль отклонения выхода от начального значения за время моделирования. |
...
Кнопка **«Добавить текущий расчёт»** закрепляет результат, присваивает ему очередное имя, сохраняет параметры формы и открывает раздел **«Сравнение»**. Кнопка доступна только после успешного расчёта. Программа хранит до шести закреплённых опытов.
...
Таблица содержит столбцы **«Опыт»**, `T`, `L`, **«Рег.»**, `Δmax`, `tуст` и `eуст`. Можно выбрать несколько строк; если они не помещаются, появляются полосы прокрутки. Если строки не выбраны, команда экспорта обрабатывает все закреплённые опыты.
...
| **«Экспорт CSV»** | Сохраняет в одном CSV-файле выбранные опыты или все, если строки не выбраны. |
...
| **«Экспорт PNG»** | Сохраняет оба текущих сравнительных графика в два PNG-файла. |
...
При выборе строк верхний график показывает переходные процессы, нижний — длительность установления. Щелчок по подписи кривой в легенде скрывает или возвращает выбранную кривую.
...
| **«Подобрать автоматически»** | Вычисляет учебные настройки по текущим `T`, `L` и типу регулятора. Включает регулятор и заполняет `K`, `Ti` и `Td`; расчёт после подбора нужно запустить отдельно. |
...
Для запуска включите хотя бы одно возмущение. Чем больше `T`, тем медленнее реакция; чем больше `L`, тем сильнее сдвиг вправо. Изменение доли показывает влияние силы возмущения. Результат отображается в общих областях графиков; легенда интерактивна.
...
| **«Td, с (для PID)»** | Фиксированное неотрицательное время дифференцирования для всей PID-карты. Для P и PI поле отключено. |
| **«Построить карту»** | Рассчитывает сетку по текущим входам, динамике, `\|u\|` и заданному значению. |

Для P строится один ряд по `Kp`; для PI и PID — плоскость `Kp × Ti`. После построения щёлкните по ячейке карты. Карточка **«Выбранная точка»** покажет значения настроек и категорию, а соседний график — переходный процесс для выбранной точки.

Кнопка **«Применить и открыть переходный процесс»** становится доступной после выбора ячейки. Она переносит параметры, включает регулятор, выполняет расчёт и открывает раздел **«Регулятор»**.
...
| **«Вариант задания»** | Выбирает номер от 1 до 30 для встроенного сценария. Для пользовательского сценария поле отключено, так как варианты предусмотрены только для защищённого встроенного комплекта. |
| **«Применить сценарий»** | Загружает цепь, возмущения, динамику, регулятор и учебное задание, затем очищает результаты. После загрузки выполните расчёт. |
| Строка **«Применён: …»** | Показывает активный сценарий или сообщает, что сценарий ещё не применён. |
...
| **«Восстановить резервную копию»** | Восстанавливает пользовательские сценарии из резервной копии. Появляется только при наличии резервной копии. |

Для каждого встроенного сценария доступны 30 воспроизводимых вариантов. Вариант 1 полностью совпадает с исходным сценарием. В вариантах 2–30 программа детерминированно изменяет величины возмущений, момент и длительность воздействия, постоянную времени `T` и запаздывание `L`; у сценария с PI-регулятором также изменяются `K` и `Ti`. Один и тот же номер всегда задаёт один и тот же набор входных данных.
...
| **«Следующий шаг»** | Включает пошаговый режим, если он выключен, и переводит маршрут к следующему этапу. После шестого этапа маршрут остаётся на последнем шаге. |
...
Все четыре прогнозных поля обязательны. Имя и вывод не влияют на оценку. Карточка **«Проверка прогноза»** показывает баллы по критериям и число оставшихся попыток. После исчерпания лимита проверить новый прогноз для текущего задания нельзя.
...
| **«Экспортировать точки CSV»** | Сохраняет временную сетку, профиль воздействия, суммарное целевое значение и все рассчитанные отклики. При включённом регуляторе добавляет регулируемый выход, ошибку `e(t)` и воздействие `u(t)`. Кодировка — UTF-8 с BOM, разделитель — точка с запятой. |
...
- щелчок по линии в легенде скрывает или возвращает выбранную кривую;
...
Окно открывается кнопкой **«Параметры модели»** в разделе **«Возмущения»**. Все шесть полей должны содержать конечные числа больше нуля.
...
Окно сразу пересчитывает расходы:
...
| **«Применить»** | Применяет все шесть значений, обновляет базовое задание и сбрасывает текущий расчёт. |
...
| Элемент | Назначение |
|---|---|
| Поле поиска | Фильтрует список по названию и описанию во время ввода. |
...
| **«Импорт JSON»** | Импортирует комплект сценариев в формате JSON v1. Совпадающие пользовательские записи обновляются. |
...
Отключённое возмущение сохраняется неактивным и не участвует в расчёте. Диапазон обеих долей — от `-0.99` до `9.99`.
...
| **«Коэффициент K»** | Определяет силу реакции регулятора на текущую ошибку. |
...
| **«Вопросы студенту (один на строку)»** | Каждая непустая строка становится отдельным вопросом. |
...
| **«Открыть в расчёте»** | Проверяет данные, закрывает редактор, переносит сценарий в главное окно и сразу выполняет расчёт. Несохранённый сценарий можно открыть для предварительного просмотра. |
...
Клавиша `Esc` закрывает редактор после проверки несохранённых изменений. Переход к другому сценарию также запрашивает подтверждение, если текущая форма изменена.
...
- параметры `T`, длительности, `Ti`, `|u|` и задание должны быть строго больше нуля;
- параметры `t₀`, `L`, `K` и `Td` должны быть неотрицательными;
...
Если в редакторе сценариев есть несохранённые изменения, при закрытии главного окна приложение сначала предлагает отменить их. Пока пользователь не подтвердит закрытие редактора, приложение не завершит работу.
...
Время установления отсчитывается до момента, после которого выход остаётся в полосе ±5% вокруг установившегося значения.
